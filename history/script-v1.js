...
async function detect(){
  ...
  if(glInfo){
    ...
    // 移动 GPU 词（弱：辅助区分移动）
    if(r.includes('adreno') || r.includes('mali') || r.includes('powervr')){
      if(isTouchy) vote(['android'], 4, '移动 GPU（Adreno/Mali/PowerVR）且为触控环境', detail);
    }
  ...
  function breakTieWithUA(candidates){
    try{
      ...
      // Android
      if(/android/.test(ua)){
        const hit = pick('android'); if(hit) return {hit, reason:'UA含 android'};
      }
      // iPhone/iPod 明确 iOS
